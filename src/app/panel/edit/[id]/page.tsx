...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.nameEs.trim() || !form.nameEn.trim())
      return setError(language === "es" ? "Nombre obligatorio" : "Name required");
    ...
    await fetch("/api/products", {
      ...
      body: JSON.stringify({ id, name: { es: form.nameEs, en: form.nameEn }, price: priceNum, categoryId: form.categoryId }),
    });
    router.push("/panel");
    router.refresh();
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-2xl mx-auto p-5">
        <h1 className="text-2xl font-bold mb-5">{language === "es" ? "Editar Producto" : "Edit Product"}</h1>
